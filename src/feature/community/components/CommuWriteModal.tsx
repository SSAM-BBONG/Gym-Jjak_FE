...
export default function CommuWriteModal({ isModal, closeModal, activeModal }: TrainerDetailModal) {
    ...
    return (
        <section
            ...
            onClick={closeModal} >
            <form
                className="bg-gradient-to-br from-[#101828] to-[#000] w-4/5 h-100 sm:w-md sm:h-100 md:w-lg md:h-100 lg:w-3xl lg:h-150 rounded-2xl border border-[#1E2939] z-1000 fixed top-1/2 left-1/2 p-6 flex -translate-x-1/2 -translate-y-1/2 flex-col justify-between
                overflow-y-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"
                onClick={(e) => e.stopPropagation()}>
                <article>
                    <div className="flex justify-between border-b-[#1E2939] border-b items-center pb-8 pt-2">
                        <h3 className="font-bold text-base md:text-lg text-[#E8EAF0]"> 새 게시글 작성 </h3>
                ...
                <div className="flex flex-col items-baseline mt-8">
                    <label className="font-bold md:text-xl text-[14px] text-white"> 카테고리</label>
                    <div className="text-black text-[14px] font-extrabold bg-[#BFFF0B] px-4 py-2 rounded-[10px] mb-6 mt-3"> 자유게시판 </div>
                    <div className="flex flex-col w-full ">
                        <label className="font-bold md:text-xl text-[14px] text-white"> 제목</label>
                        <input
                            type="text"
                    ...
                    <div className="flex flex-col w-full">
                        <label className="font-bold text-base md:text-xl text-[14px] text-white" > 내용 </label>
                        <textarea
                            placeholder="내용을 입력하세요"
                            className="text-sm md:text-base bg-[#1E2939] px-4 py-3 rounded-[10px] border-[1px] border-[#364153] text-white outline-none mb-6 mt-3"
                            rows={15} cols={50} />
                    </div>
