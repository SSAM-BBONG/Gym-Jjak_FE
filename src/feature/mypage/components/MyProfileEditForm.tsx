...
export default function MyProfileEditForm( { data, socialUser }: MyProfileEditProps) {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            <p className="text-2xl sm:text-3xl lg:text-[36px] font-black text-white"> 프로필 수정</p>
            <p className="text-xs sm:text-sm lg:text-[14px] font-normal text-[#99A1AF]"> 회원 정보를 수정하세요</p>
            ...
            my-4 md:my-8">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <input 
                        ...
                        id="profile-name"
                        placeholder="닉네임"
                        className="flex-1 min-w-0 bg-[#1E2939] border border-[#364153] px-4 py-3 rounded-[10px] text-white outline-none"/>
                    <button 
                        onClick={handleNicknameDuplicationCheck}
                        type="button"
                        className="flex-none min-w-[84px] text-[14px] sm:text-[16px] font-medium text-white px-3 sm:px-4 py-3 bg-[#364153] rounded-[10px]">
                        중복 확인 
                    </button>
                    </div>
