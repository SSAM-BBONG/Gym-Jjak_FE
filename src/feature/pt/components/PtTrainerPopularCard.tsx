    ...
    return (
        ...
        ">
            ...
            <div className="flex flex-col p-4">
                <p className="truncate text-[18px] font-extrabold text-white"> {data.title}</p>
                <p className="text-[12px] font-normal text-[#99A1AF]"> {data.trainerName} </p>
                <div className="flex items-center justify-between gap-2">
                    <p className="min-w-0 truncate text-[12px] font-normal text-[#6A7282]"> {data.organizationName} </p>
                    <p className="shrink-0 whitespace-nowrap text-[14px] font-extrabold text-[#BFFF0B]"> {data.price.toLocaleString()}원 </p>
                </div>
            </div>
        </div>
    );
