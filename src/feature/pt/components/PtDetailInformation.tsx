...
export default function PtDetailInformation( { response, trainerInformation}: PtDetailInformationProps) {
    return (
            <div
                ...
                rounded-[14px]  
                p-6">
                <p className="text-[36px] font-black text-white"> {response.title} </p>
                <div className="flex justify-between items-center">
                    <div className="flex gap-3 items-center">
                        <div className="text-[#BFFF0B] text-[20px]"> ★ </div>
                        <p className="text-[18px] font-extrabold text-white">
                            {trainerInformation.averageRating}  <span className="text-[14px] font-normal text-[#99A1AF]">({trainerInformation.reviewCount}개)</span>
                        </p>
                    </div>
                    <div className="flex flex-col gap-1">
                        <p className="text-[36px] font-black text-[#BFFF0B]"> {response.price.toLocaleString()}원</p>
                        <p className="text-[14px] font-normal text-[#99A1AF] text-right"> {response.totalSessionCount}회 </p>
                    </div>
                </div>
            </div>
    );
