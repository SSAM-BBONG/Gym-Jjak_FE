...
export default function PtCourseEditForm({ course }: { course: PtCourseDetailData }) {
  ...
  const { register, handleSubmit, setValue, formState: { errors, isSubmitting } } = useForm<PtRegistFormValue>({
    resolver: zodResolver(ptRegistSchema.omit({ thumbnailFile: true, organizationId: true })) as unknown as Resolver<PtRegistFormValue>,
    defaultValues: {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="mt-6 flex flex-col gap-6">
      <PtRegistPreview setValue={setValue} error={errors.thumbnailFile?.message} initialThumbnailUrl={course.thumbnailUrl} />
