...
import AdminHeaderAuthArea from "./AdminHeaderAuthArea";
import AdminMobileNavMenu from "./AdminMobileNavMenu";

interface AdminHeaderProps {
    userInf?: MyTokenPayload;
}

export default function AdminHeader({ userInf }: AdminHeaderProps) {
    return (
        <header className="fixed top-0 left-0 z-9999 flex h-17.5 w-full items-center justify-between border-b border-b-[#1E2939] bg-black px-5 sm:px-10">
            <div className="flex gap-2">
                <AdminMobileNavMenu />
                <Link href="/admin/dashboard/users" className="flex items-center gap-3">
                    <div className="hidden sm:block flex size-10 items-center justify-center rounded-[10px]">
            ...
            <div className="flex items-center gap-4 sm:gap-6">
                ...
                <AdminHeaderAuthArea userInf={userInf} />
            </div>
        </header>
    );
}
