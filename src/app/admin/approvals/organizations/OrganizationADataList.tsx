...
export default function OrganizationADataList({ organizations, totalPage, page, sort }: OrganizationApplicationDataListProps) {
    ...
    return (
        <div>
            ...
            <section className="bg-[#1E2939] border-[#364153] border-separate border-spacing-0 md:border mt-4 sm:mt-5 lg:mt-6 rounded-md w-full ">
                ...
                {sortedOrganizations.map(organization => <OrganizationADataItem organization={organization} key={organization.organizationApplicationId} />)}
            ...
            <AdminPagination url={`approvals/organizations`} page={page} totalPage={totalPage} sort={sort} />
        </div>
    );
}
