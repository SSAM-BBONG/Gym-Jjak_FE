...
export default async function TrainerReportDetailPage({ params }: TrainerReportDetailPageProps) {
  ...
  if (!Number.isInteger(reportId) || reportId < 1) {
    return (
      <div className="px-4 py-6 sm:px-10 sm:py-8 md:px-20 md:py-10 lg:px-40 lg:py-10 text-[#99A1AF]">유효하지 않은 리포트입니다.</div>
    );
  }
  ...
  if (!result.success) {
    return (
      <div className="flex flex-col gap-4 px-4 py-6 sm:px-10 sm:py-8 md:px-20 md:py-10 lg:px-40 lg:py-10">
        <p className="text-base md:text-lg lg:text-xl font-bold text-white">리포트를 불러올 수 없습니다.</p>
  ...
  return (
    <main className="px-4 py-6 sm:px-10 sm:py-8 md:px-20 md:py-10 lg:px-40 lg:py-10">
